...
def extract_support_notes(target_source_path: str, target_sheet: str, table_name: str,  **kwargs):
    ...
    for root, dirs, files in os.walk(target_source_path):
        for file in files:
            if file == "PBI DATA - Copy.xlsx":
                ...
                idx_cols = ['id', 'support_providers']
                excl_cols = ['average', 'units', 'total','successful_submission','success%']
                on_cols = [c for c in polarsDF.columns if c not in idx_cols + excl_cols]
