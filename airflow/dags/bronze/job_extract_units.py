...
def extract_units(target_source_path: str, target_sheet: str, table_name: str,  **kwargs):
        ...
        for file in files:
            if file == "PBI DATA - Copy.xlsx":
                ...
                polarsDF = polarsDF.filter(
                    (pl.col('support_providers') != 'Average')
                )
                polarsDF = polarsDF.drop_nulls(subset=['support_providers'])
                
                # just select support providers and units for this table
                polarsDF = polarsDF.select(
                    pl.col('support_providers'),
                    pl.col('units')
                )
                              
                # add snapshot metadata
                polarsDF = polarsDF.with_columns(
