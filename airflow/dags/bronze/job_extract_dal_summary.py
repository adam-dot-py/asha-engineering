...
def extract_dal_summary(target_source_path: str, target_sheet: str, table_name: str,  **kwargs):
    ...
    for root, dirs, files in os.walk(target_source_path):
        for file in files:
            if file == "PBI DATA - Copy.xlsx":
                ...
                new_columns = [col.replace(' ', '_').replace('-', '_').lower() for col in polarsDF.columns]
