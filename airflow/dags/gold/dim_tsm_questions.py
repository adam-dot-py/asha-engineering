...
def create_dimension_table(schema, gold_table, **kwargs) -> None:
    ...
    # write to duckdb
    con.sql(f"CREATE OR REPLACE TABLE {schema}.{gold_table} AS SELECT * FROM df;")
    con.sql(f"COPY {schema}.{gold_table} TO '/home/asha/airflow/dags/gold/dimensions/{gold_table}.parquet' (FORMAT PARQUET);")
    con.close()
    
if __name__ == '__main__':
    
    schema = 'main_gold'
    gold_table = 'dim_tsm_questions'

    create_dimension_table(schema, gold_table)